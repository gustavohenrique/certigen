...
func bindEnvs(spec interface{}) error {
	tagName := "env"
	infos, err := gatherInfo(tagName, spec)
...
func gatherInfo(tagName string, spec interface{}) ([]varInfo, error) {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		if f.Kind() == reflect.Struct {
			if decoderFrom(f) == nil && setterFrom(f) == nil && textUnmarshaler(f) == nil && binaryUnmarshaler(f) == nil {
				...
				embeddedInfos, err := gatherInfo(tagName, embeddedPtr)
				if err != nil {
					return nil, err
				}
